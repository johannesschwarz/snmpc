...
int main(int argc, char **argv)
{
    ...
    WheelDynBicycle wheeldyn_bicycle;                       // dynamics object
    Function ode_rhs = wheeldyn_bicycle.getDynamics();  // get the RHS of the robot ODE
    ...
    polympc::nmpf<WheelDynBicycle, Path, dimx, dimu, n_segments, poly_order> controller(tf);  // nmpf object
    ...
    const double omega_wheel_max = v_max / 0.3;
    const double omega_wheel_min = 0;
    // states: [vx, vy, omega, x, y, theta, owf, owr, theta_aug, theta_dt_aug]
    DM lbx = DM::vertcat({v_min, -v_y_max, -omega_max, -xy_max, -xy_max, -theta_max, omega_wheel_min, omega_wheel_min, -DM::inf(), -DM::inf()});
    DM ubx = DM::vertcat({v_max, v_y_max, omega_max, xy_max, xy_max, theta_max, omega_wheel_max, omega_wheel_max, DM::inf(), DM::inf()});
    ...
    // set initial state
    state = DM::vertcat({10.0, 0, 0, 0, 0, M_PI_2, 10.0 / 0.31, 10.0 / 0.35});
